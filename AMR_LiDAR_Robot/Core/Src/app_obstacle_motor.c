...
void App_ObstacleMotor_Task(void)
{
    ...
    APP_LOG("APP OBS GROUND: state=%s reason=%s decision=%s front=%s front_wide=%s obs_front=%s source=%s left=%s right=%s hold_ms=%lu backup_cooldown_ms=%lu escape_elapsed_ms=%lu",
